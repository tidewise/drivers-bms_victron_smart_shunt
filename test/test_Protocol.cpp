...
TEST_F(ProtocolTest, it_accepts_a_packet_and_correctly_parse_it)
{
    ...
    ASSERT_NEAR(feedback.deepest_discharge_depth, 120, 1e-3);
    ASSERT_NEAR(feedback.last_discharge_depth, 8, 1e-3);
    ASSERT_NEAR(feedback.average_discharge_depth, 10, 1e-3);
    ASSERT_EQ(feedback.charge_cycles_number, 3000);
    ASSERT_EQ(feedback.full_discharges_number, 10);
    ASSERT_NEAR(feedback.cumulative_charge_drawn, 35, 1e-3);
